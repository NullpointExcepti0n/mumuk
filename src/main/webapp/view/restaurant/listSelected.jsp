...
<body onload="bmChk()">
	<div class="category">
		<section class="food_category">
			<div class="food_cate">
				<button class="categoryBtn" onclick="location.href='restaurantList.ha'">
			...
			<section class="restaurant_list">
				<c:forEach var="restaurant" items="${list }" varStatus="status">
					<article class="restaurant">
						<div class="thumbnail">
							<a class="restaurnatInfo" href="restaurantInfo.ha?rno=${restaurant.rno }"> <img
				...
				<div class="paging">
					...
					<c:if test="${startPage > PAGE_PER_BLOCK }">
						<button
							onclick="location.href='listSelected.ha?pageNum=${startPage-1}&selectedFood=${selectedFood }&sort=${sort }'" class="btn5">이전</button>
					</c:if>
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<button
							onclick="location.href='listSelected.ha?pageNum=${i}&selectedFood=${selectedFood }&sort=${sort }'" class="btn4">${i } </button>
					</c:forEach>
					<!-- 아직 보여줄 것이 남아있다면 -->
					<c:if test="${endPage < totalPage }">
						<button
							onclick="location.href='listSelected.ha?pageNum=${endPage+1}&selectedFood=${selectedFood }&sort=${sort }'" class="btn5">다음</button>
					</c:if>
				</div>
			</section>
		</div>
